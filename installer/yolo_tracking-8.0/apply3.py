...
class Detect:
    def __init__(self, weights=ROOT / 'weights/best.pt', data=ROOT / 'yolov5/data/prohibited.yaml',
                 reid_weights=WEIGHTS / 'osnet_x0_25_msmt17.pt',  # model.pt path,
                 tracking_method='bytetrack', half=False, yolov8_weights="./weights/trunk_best.pt"):
        ...
        self.model2 = YOLO(yolov8_weights, task='detect')
    ...
    def detect(self, img, threshold, isDetect):
        # print(img.shape)
        t1 = time_sync()
        # Run inference
        # 开始预测
        ...
        self.count += 1
        detect_res = self.model2(img, verbose=False, conf=0.5, classes=0, max_det=16)[0]
        # print(detect_res.boxes.cls,detect_res.boxes.conf)

        pred = detect_res.boxes.data
        self.curr_frames[0] = im0
        outputs = self.tracker_list[0].update(pred.cpu(), self.curr_frames[0])
        new_xyxys = []
        ...
        if outputs is not None and len(outputs) > 0:
            for j, (output, conf) in enumerate(zip(outputs, pred[:, 4])):
                xyxy = output[0:4]
                id = int(output[4])
                cls = int(output[5])
                xyxy = [int(i) for i in xyxy]
                new_xyxys.append(xyxy)
                images.append(img[xyxy[1]:xyxy[3], xyxy[0]:xyxy[2]])
                max_width = max(xyxy[2] - xyxy[0], max_width)
                max_height = max(xyxy[3] - xyxy[1], max_height)
                track_result[id] = xyxy
                ids.append(id)
        else:
            return track_result, {}
        # 0检测 其它不检测
        ...
        for image in images:
            ...
            detect_images.append(new_image)
        stacked_images = np.stack(detect_images, axis=0)
        scale_shape = stacked_images[0].shape
        detect_images = []
        for i in range(stacked_images.shape[0]):
            detect_image = letterbox(stacked_images[i], self.imgsz, self.model.stride, auto=self.model.pt)[0]
            detect_image = detect_image.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            detect_images.append(detect_image)
        im = np.ascontiguousarray(detect_images)
        ...
        for i, det in enumerate(pred):  # per image 每张图片
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    cls = int(cls)
                    labels.append({'label': self.cls_names[cls], 'xyxy': xyxy, 'conf': float(conf), 'cls': cls})

            labels_r[id] = labels
        dt[2] += time_sync() - t3

        return track_result, labels_r
