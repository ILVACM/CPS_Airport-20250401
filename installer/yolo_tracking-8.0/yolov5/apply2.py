...
class Detect:
    def __init__(self, weights=ROOT / 'best.pt', data=ROOT / 'data/prohibited.yaml',reid_weights=WEIGHTS / 'osnet_x0_25_msmt17.pt',  # model.pt path,
    ...
    def detect(self, img,input_view):
        # print(img.shape)
        t1 = time_sync()
        # Run inference
        # 开始预测
        ...
        # Process predictions
        labels_r = []
        # annotator_up = Annotator(im0[1], line_width=3, example=str(self.cls_names))
        # annotator_down = Annotator(im0[0], line_width=3, example=str(self.cls_names))
            ...
            if len(det):
                ...
                if len(self.outputs[i]) > 0:
                    ...
                    for j, (output, conf) in enumerate(zip(self.outputs[i], det[:, 4])):
                        ...
                        labels.append({'id':id,'label': self.cls_names[cls], 'xyxy': xyxy, 'conf': conf})

                # for *xyxy, conf, cls in reversed(det):
                #     c = int(cls)  # integer class
                #     # label = f'{self.model.names[c]} {conf:.2f}'
                #     label = f'{self.cls_names[c]} {conf * 100:.2f}%'
                #
                #     # im0=draw_china(im0,xyxy,3,self.font, label, color=colors(c, True))
                #     # if i==0:
                #     #     annotator_up.box_label(xyxy, label, color=colors(c, True))
                #     # else:
                #     #     annotator_down.box_label(xyxy, label, color=colors(c, True))
                #
                #     # cv2.imshow("11",im0)
                #     # cv2.waitKey(0)
                ...
                #     labels.append({'label': label,'xyxy':xyxy,'conf':conf})

                labels_r.append(labels)
                self.prev_frames[i] = self.curr_frames[i]
                # if i==0:
                #     cv2.imwrite('./images/' + str(self.count) + '_up.jpg', annotator_up.result())
                # else:
                #     cv2.imwrite('./images/' + str(self.count) + '_down.jpg', annotator_down.result())
        # cv2.imshow("up",annotator_up.result())
        # cv2.imshow("down", annotator_down.result())
        # cv2.waitKey(2)
        dt[2] += time_sync() - t3
