            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        c = int(cls)  # integer class
                        label = None if hide_labels else (cls_names[c] if hide_conf else f'{cls_names[c]} {conf*100:.2f}%')
                        # label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf*100:.2f}%')
                        # label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
                        # annotator.box_label(xyxy, label, color=colors(c, True))
                        im0=draw_china(im0,xyxy,line_thickness,font, label, color=colors(c, True))
                    if save_crop:
