...
class Detect:
    def __init__(self, weights=ROOT / 'yolov5/best.pt', data=ROOT / 'yolov5/data/prohibited.yaml',reid_weights=WEIGHTS / 'osnet_x0_25_msmt17.pt',  # model.pt path,
        ...
        self.y8model = YOLO('weights/yolov8_best.pt')
        ...
        self.ort_sess = ort.InferenceSession('yolov5/best.onnx',providers=['TensorrtExecutionProvider', 'CUDAExecutionProvider', 'CPUExecutionProvider'])
    def detect(self, img,input_view):
        ...
        pred = [self.y8model(im, augment=self.augment, visualize=self.visualize)[0].boxes.data]



        # pred = self.ort_sess.run(None, {'images': im.cpu().numpy()})
        # pred = torch.from_numpy(np.array(pred)).to(self.device)
        # NMS
            ...
            if len(det):
                ...
                if len(self.outputs[i]) > 0:
                    ...
                    for j, (output, conf) in enumerate(zip(self.outputs[i], det[:, 4])):
                        ...
                        labels.append({'id':id,'label': self.cls_names[cls], 'xyxy': xyxy, 'conf': float(conf)})
                    print(labels)
